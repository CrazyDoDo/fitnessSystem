...
//点击按钮,实现会员卡挂失	
		...
		if(loss=="挂失"){
			$.post("membrseCardLoss","membersId="+membersId,memLeave);
			function memLeave(data){
				if(data==2){
					$.MsgBox.Alert("消息", "挂失成功"); 
					$("#ka").text("");
					$("input[name='cardName']").val("");
				}else if(data==1){
					$.MsgBox.Alert("消息", "未找到该会员"); 
				}else{
					$.MsgBox.Alert("消息", "挂失失败,请联系管理员"); 
				}
			}
		}else{
					$.MsgBox.Alert("消息", "若需挂失会员卡,请点击是"); 
		}	
	  });	
    });
</script>
<body>
	...
	<div class="container">
		<div class="content">
			<div class="easyui-tabs1" style="width:100%; height:100%;">
		      <div title="会员卡挂失" data-options="closable:false" class="basic-info">
		      	<div class="column"><span class="current">会员卡挂失</span></div>
	...
	<div class="div">
    	...
    	<table>
            ...
            <tr>
            	<td class="left">是否挂失:</td>
                <td><input type="radio" class="vertical" name="loss" value="挂失" />是
                	<input type="radio" class="vertical" name="loss" value="正常" checked="checked" />否
                </td>
            </tr>                                                             
        </table>
