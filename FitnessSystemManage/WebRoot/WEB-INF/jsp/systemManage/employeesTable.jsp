	...
	<div class="container">
		<div class="content">
			<div class="easyui-tabs1" style="width:100%;height:100%;">
				<div title="员工列表" data-options="closable:false" class="basic-info">
					<div class="column">
						<span class="current">员工列表</span><img id="update"
							src="${pageContext.request.contextPath}/statics/images/shuxin2.jpg"
							width="40" height="17"
							style="margin:0px 0px 0px 70px; cursor:pointer">
					</div>
					<input type="button" id="addEmployees" value="+添加员工" style="width:100px; height:25px; margin-bottom:5px"/>  
					<form action="#" method="post" id="formInfo" style="margin-bottom:5px;">
						<span style="font-size:12px">工号:</span> <input type="text"
							name="employeesId" size="20px" style="height:20px;"> <span
							style="font-size:12px">姓名:</span><input type="text" size="20px"
							name="name" style="height:20px;"></input>
							...
							<thead>
								<tr class="headings" style="background-color:#828282">
									...
									<th class="bulk-actions" colspan="9"><a class="antoo"
										style="color:#fff; font-weight:500;">删除员工(<span
...
<script type="text/javascript">
	...
	function nextPages(pageIndex) {
		...
		$.post("fitness/emoloyeesController/getEmployeesList.json", "pageIndex="
				+ pageIndex + "&" + formInfo, getEmployeesList, "JSON");
		function getEmployeesList(date) {
			var employeesList = date.employeesList;
			for ( var i = 0; i < employeesList.length; i++) {
				$(".infoList")
						.append(
								"<tr class='even pointer'>"
										+ "<td style='padding-top: 5px'><input type='checkbox' aa='1' class='flat' name='table_records' value='"
										+ employeesList[i].employeesId + "'></td>"
										+ "<td class='update'>"
										+ employeesList[i].employeesId + "</td>"
										+ "<td class='update'>"
										+ employeesList[i].name
										+ "</td>" + "<td class='update'>"
										+ employeesList[i].username
										+ "</td>" + "<td class='update'>"
										+ employeesList[i].phone
										+ "</td>" + "<td class='update'>"
										+ employeesList[i].typeName
										+ "</td>" + "<td class='update'>"
										+ employeesList[i].stratDate
										+ "</td></tr>");
	...
	function deletePersonalDrop() {
		$(".bulk_action input[name='table_records']:checkbox:checked").each(
				function() {
					...
					$.post("fitness/emoloyeesController/deleteEmployees.json",
							"employeesId=" + employeesId, deleteEmployees,
							"JSON");
	...
	$("#addEmployees").click(function(){
						var title = "添加员工信息";
						var url = "fitness/emoloyeesController/goAddEmployees";
						function addTab(title, url) {
							var content = "<div title='添加员工信息' data-options='closable:true'  class='basic-info'><div class='column'><span class='current'>添加员工信息</span></div>"
	...
	$(".update").live("click",function(e) {
						var title = "更改员工信息";
						var url = "fitness/emoloyeesController/goUpdateEmployees?employeesId="+$(this).parent("tr").find(".flat").val();
						function addTab(title, url) {
