...
/*用于定位当前页码*/
	...
	function nextPages(pageIndex) {
		$(".pagination").html("");
		$(".memcard").html("");
		$.post("fitness/siJiaoDengJi/getPersonalLeverList.json", "pageIndex=" + pageIndex , showPersonalLever, "JSON");
		function showPersonalLever(date) {
			var personalLeverList = date.personalLeverList;
			for ( var i = 0; i < personalLeverList.length; i++) {
				$(".memcard")
						.append(
								"<tr class='even pointer' name='fitness/siJiaoDengJi/goXiugaidengji?leverId="
										+ personalLeverList[i].leverId
										+ "'>"
										+ "<td><input type='checkbox' aa='1' class='flat' name='table_records' value='"
										+ personalLeverList[i].leverId + "'></td>"
										+ "<td class='update'>"
										+ (i+1)+ "</td>"
										+ "<td class='update'>"
										+ personalLeverList[i].personalLever+ "</td>"
										+ "<td class='update'>"
										+ personalLeverList[i].personalPrice + "</td>"
										+ "<td class='update'>"
										+ personalLeverList[i].empName + "</td></tr>");
	...
	$(".update").live("click",function(e){
						...
						var content = "<div title='修改私教等级' data-options='closable:true'  class='basic-info'>"
